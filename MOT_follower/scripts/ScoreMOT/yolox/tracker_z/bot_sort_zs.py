...
class BoTSORT(object):
    ...
    def update(self, output_results, img):
        ...
        if self.args.with_reid:
            ...
            emb_dists = matching.mult_embedding_distance(strack_pool, detections)
            ...
            # dists = matching.fuse_iou(raw_emb_dists, strack_pool, detections)
            # emb_dists = dists
        ...
        strack_score = np.array([np.clip(strack.score_kalman, 0.1, 0.6) for strack in r_tracked_stracks])
        det_score = np.array([det.score for det in detections_second])
        dists += (abs(np.expand_dims(strack_score, axis=1).repeat(dists.shape[1], axis=1) - det_score) * 1.0)
